...
\lstdefinelanguage{LaTeX}[LaTeX]{TeX}{
  moretexcs={chapter,text,RequirePackage,includegraphics,bf,xymatrix,ltxinline,href,RequirePackage}
}

\lstdefinestyle{latexml}{language=LaTeX,
  texcsstyle=*{\color{blue}\bfseries},
  texcsstyle=*[2]{\color{MidnightBlue}\bfseries},
  moretexcs=[2]{LaTeXML,iflatexml,lxAddClass,lxWithClass,lxBeginTableHead,lxEndTableHead,lxFcn,lxID,lxPunct,lxContextTOC,lxNavbar,lxHeader,lxFooter}
}

\lstdefinestyle{bookml}{style=latexml,
  moretexcs=[2]{bmlImageEnvironment,bmlHTMLEnvironment,bmlRawHTML,bmlDescription,bmlPlusClass,bmlHTMLInlineEnvironment,bmlAltFormat,bmlDisableMathJax,<}
}
...
\date{5\textsuperscript{th} September 2026}
...
\begin{center}
  ...
  Formats: \href{https://vlmantova.github.io/bookml/}{GitBook} (html), \href{plain/}{plain} with Latin Modern (html), \href{https://vlmantova.github.io/bookml/docs.pdf}{PDF}, \href{https://github.com/vlmantova/bookml/blob/docs/docs.tex}{\LaTeX{} source}.
...
\begin{itemize}
  \item simple installation: simply drop the \texttt{bookml/} directory next to the files to be compiled and copy \texttt{GNUmakefile} in the same place (well, uhm, that is a bit of a lie: you need to install \LaTeXML{} first! see \hyperref[get-started-on-your-device]{Getting started on your device});
  \item Overleaf and GitHub integration: BookML can be used with Overleaf by adding a single file to your project or using the \href{https://www.overleaf.com/latex/templates/bookml-template/nycbmrhpscpx}{BookML template}, and linking it to GitHub (which requires a paid Overleaf subscription), or by directly putting your files on GitHub (no subscription required);
  \item accessible and mobile friendly output: virtually identical to the \href{https://bookdown.org/yihui/bookdown/html.html#gitbook-style}{GitBook style} of \href{https://bookdown.org}{bookdown}, including font selection and dark mode, tweaked to meet the \href{https://www.w3.org/TR/WCAG21/}{Web Content Accessibility Guidelines 2.1} level AA;
  \item fully automated (re-)compilation based on which files have changed on disk, powered by \href{https://www.gnu.org/software/make/}{GNU Make}: just run \cmdinline{make} to zip together all PDF and HTML outputs from all the main \texttt{.tex} files in the directory (one package per main \texttt{.tex} file);
  \item high quality conversion of external EPS and PDF figures to SVG via \href{https://mupdf.readthedocs.io/en/latest/tools/mutool.html}{mutool}, \href{https://poppler.freedesktop.org/}{pdftocairo}, \href{https://inkscape.org/}{Inkscape} or \href{https://dvisvgm.de}{dvisvgm}, rather than ImageMagick used by LaTeXML;
  \item transparent generation of SVG images from \href{https://tikz.net/}{\tikzname} pictures, \href{https://ctan.org/pkg/animate}{animate} animations, \Xy-matrices, and virtually any other picture-like environment: just add a few lines of code in the preamble
...
\fi
    \end{lstlisting}
  \item alternative text for images straight from \LaTeX:
    \begin{lstlisting}[style=bookml]
\usepackage{bookml/bookml}\bmlImageEnvironment{tikzpicture}
    \end{lstlisting}
  \item arbitrary \HTML{} content in \LaTeX{}, such as collapsible tags:
    \begin{lstlisting}[style=bookml]
\usepackage{bookml/bookml}
...
\<details>
...
\</details>
    \end{lstlisting}
  \item declare alternative PDF versions (for instance sans serif, large print):
    \begin{lstlisting}[style=bookml]
\bmlAltFormat{notes-large-print.pdf}{PDF (large print)}
% you must provide notes-large-print.tex
    \end{lstlisting}
    the files will be automatically compiled and included in the `Download' menu of bookdown (see the \href{https://github.com/vlmantova/bookml/releases/latest/download/example.zip}{example} for more info)
  \item \href{https://scorm.com/}{SCORM} support: running \cmdinline{make} creates valid \texttt{SCORM.*.zip} packages supported by most Learning Management Systems (if not, please submit an issue!).
\end{itemize}
...
\begin{enumerate}
  \item Install the bare minimum prerequisites: \href{https://dlmf.nist.gov/LaTeXML/get.html}{LaTeXML} (0.8.8 or later) and \href{https://www.gnu.org/software/make/}{GNU Make} (minimum 3.81, but later versions are considerably faster), on top of a working \TeX{} installation, such as \TeX{} Live or MiK\TeX{}.
...
\cellcolor{red}\color{white} Required\bmlRawHTML{ }\phantom{:} & \cellcolor{red}\bmlRawHTML{} \\
...
\cellcolor{red}\color{white} Optional: & \cellcolor{red}\textcolor{white}{for any image handling, including BookML images} \\
  Image::Magick: & \textcolor{red}{NOT FOUND}                                                    \\
\multicolumn{2}{X}{\textcolor{magenta}{\tt -{}-{}- PDFTOSVG\_CONVERTER is set to `auto' -{}-{}-}} \\
\multicolumn{2}{X}{\textcolor{magenta}{\tt -{}-{}- EPSTOSVG\_CONVERTER is set to `auto' -{}-{}-}} \\
  Ghostscript:   & \textcolor{OliveGreen}{9.25 OK} (BookML images, EPS/PDF to SVG via dvisvgm) \\
  dvisvgm:       & \textcolor{OliveGreen}{3.4.3 OK} (PDF to SVG via mutool)) \\
  dvisvgm/libgs: & \textcolor{OliveGreen}{9.25 OK} (BookML images, EPS to SVG, PDF to SVG via dvisvgm)  \\
  mutool:        & \textcolor{OliveGreen}{1.23.0 OK} (PDF to SVG via mutool) \\
  pdftocairo:    & \textcolor{red}{NOT FOUND} (PDF to SVG via pdftocairo) \\
  inkscape:      & \textcolor{red}{NOT FOUND} (EPS/PDF to SVG via Inkscape) \\
\cellcolor{red}\color{white} Optional: & \cellcolor{red}\textcolor{white}{BookML images (\textbackslash{}bmlImageEnvironment and \textbackslash{}begin\{bmlimage\})} \\
  preview.sty:   & \textcolor{OliveGreen}{14.0.6 OK}                                             \\
\cellcolor{red}\color{white} Optional: & \cellcolor{red}\textcolor{white}{misc} \\
  texfot:        & \textcolor{OliveGreen}{1.48 OK} (hide irrelevant LaTeX messages)    \\
  curl:          & \textcolor{OliveGreen}{8.13.0 OK} (update BookML with `make update')
\end{tabularx}
...
\begin{foldedframe}[open=""]{Videos}\label{videos}
  You can inject arbitrary \HTML{} using \ltxinline|\bmlRawHTML{HTML here}| or \ltxinline|\<div>TeX here\</div>|. That is enough to insert videos, for example by typing the YouTube embed code. Special characters may need to be escaped with a backslash, for instance replace \ltxinline|%| with \ltxinline|\%|, and you must follow the \XML{} syntax, so an attribute \htmlinline|<iframe allowfullscreen>| will need to be written as \htmlinline|<iframe allowfullscreen="">|.
  \begin{lstlisting}[style=bookml]
...
\fi
  ...
  The \HTML{} tags can be written in normal HTML5 syntax, but they must be closed (so remember to write \ltxinline|\<br/>| rather than \ltxinline|\<br>|). See \hyperref[reference-manual]{Reference manual} for more details.
\end{foldedframe}
...
\begin{foldedframe}{Split by chapter instead of section, or not split at all}\label{split-by-chapter}
  ...
  \begin{lstlisting}
lecturenotes_SPLITAT=chapter
problemsheet1_SPLITAT=
\end{lstlisting}
  to the end of \texttt{GNUmakefile}. See \hyperref[reference-manual]{Reference manual} for more details.

  By default, all files are split by section.
\end{foldedframe}

\begin{foldedframe}{Change MathJax version}\label{mathjax-versions}
  The current default for BookML is MathJax 4, which provides the best rendering quality, speed, and accessibility features. However, if you need to go back to a previous version, use
  \begin{lstlisting}[style=bookml]
\usepackage[mathjax=3]{bookml/bookml}
  \end{lstlisting}
\end{foldedframe}
...
\bmlAltFormat{docs.pdf}{}
  ...
  Please note that \texttt{docs.large.tex} must \emph{not} contain the string \ltxinline|\documentclass| or it will be compiled by itself into separate \HTML{}, zip, and SCORM. Otherwise, change the value of the variable \texttt{SOURCES}.
...
\begin{foldedframe}{Some PDF and EPS figures look odd or prevent compilation}\label{odd-pdf-eps-figures}
  By default, BookML uses \texttt{dvisvgm} or \texttt{inkscape} to convert EPS images to SVG, and one of \texttt{mutool}, \texttt{pdftocairo}, \texttt{inkscape}, \texttt{dvisvgm} to convert PDF images to SVG. BookML will determine automatically the best option available on your machine. The output of \cmdinline|make detect| will show you what is available.
...
\item[\texttt{mathjax=<\textit{number}>}] Select which MathJax version to use from 2, 3, 4 (default).
...
\item[\texttt{draft}] Do not compile and include the alternative formats and do not generate the BookML images. BookML images will be replaced by file names.
\item[\texttt{final}] Compile and include the alternative formats and generate the BookML images (default behaviour if neither of <code>draft</code>, <code>final</code> is specified).
\end{description}
...
\item[\texttt{\textbackslash{}<}] Open or close an \HTML{} tag, as in \ltxinline|\<span class="example">some \LaTeX{} code\</span>|. The content between the tags is normal \LaTeX{} code. If necessary, the behaviour of each tag can be changed with the \ltxinline|\bmlHTML*Environment| commands.  Tags will normally generate an additional \htmlinline|<p>...</p>| tag, unless the HTML5 spec says that they can only contain `phrasing content' or that they are `transparent', or if their direct descendants are also HTML tags. The text between \ltxinline|\<| and \ltxinline|>| is parsed following the HTML5 spec, except that all tags need to be closed, \ltxinline|/>| closes non-void tags, and only the basic XML enties are supported.
...
\item[\texttt{\textbackslash{}bmlHTMLInlineEnvironment\{<\textit{tag}>}\}] Introduce or redefine an \HTML{} tag environment which accepts `phrasing content' only or is `transparent'.
\item[\texttt{\textbackslash{}bmlRawHTML\{<\textit{html}>}\}] Insert \texttt{\texttt{<\textit{html}>}} directly in the output document, after expanding all the \TeX{} macros. Special characters are handled like in \ltxinline|\href| from the hyperref package: you should use \ltxinline|\\| to enter a literal backslash, while other special characters can be escaped as \ltxinline|\#|, \ltxinline|\%|, \ltxinline|\&|, \ltxinline|\&| when necessary. When the command appears in the preamble, \texttt{<\textit{html}>} will be part of the head and will be copied in every output page. \texttt{<\textit{html}>} must be written in valid XML syntax, with either no namespace or the correct namespace for \HTML. The HTML5 syntax is currently \textbf{not} supported.
...
The build process accepts configuration options via Make variables, using the syntax \makeinline|VARIABLE=value| to replace a value and \cmdinline|FLAGS+=moreflags| to append more values. Changing the settings will automatically recompile the files that are affected. To apply an option only to the outputs of \texttt{file.tex}, use \makeinline|file_VARIABLE| and \makeinline|file_FLAGS| (normal variables are replaced, while \makeinline|file_*FLAGS| variables are added to the general \makeinline|*FLAGS| values). Special characters in the file name must be replaced with \makeinline|_|; for instance use \makeinline|sheet1_solutions_SPLITAT| to control the outputs of \texttt{sheet1+solutions.tex}. Comments can be added by starting the line with \makeinline|#|.

For instance, the following \texttt{GNUmakefile} specifies that the files to compile are \texttt{notes.tex} and \texttt{exercises.tex} only (instead of every \texttt{.tex} file with \ltxinline|\documentclass|), that they must be compiled to SCORM packages, and that \texttt{exercises.tex} must not be split by section. Moreover, \texttt{exercises.tex} should be complied using LuaTeX.
...
\begin{enumerate}
  \item Written in \texttt{GNUmakefile}, with each option on its own line, without indentation. You typically want to put the values before \makeinline|include bookml/bookml.mk|, for instance to avoid scanning the \texttt{.tex} files when you are overriding \makeinline|SOURCES|. Moreover, \makeinline|AUX_DIR| **must** appear **before** \makeinline|include bookml/bookml.mk|. On the other hand, adding flags with \makeinline|+=| **must** be done **after** \makeinline|include bookml/bookml.mk|.
  \item On the command line as \cmdinline|make VARIABLE=value|. Appending is not supported on the command line.
  \item In your shell environment.
  \item In \texttt{GNUmakefile} applied directly to the target, like \makeinline|file.pdf: LATEXMKFLAGS = -pdflua|. This is discouraged: it behaves consistently **only if applied to the ultimate affected target**. For instance, \makeinline|SPLITAT| must be applied to \makeinline|$(AUX_DIR)/html/file/index.html|. If applied to \texttt{file.zip} by mistake, the variable may or may not take effect depending on its value on \texttt{SCORM.file.zip}, potentially leading to an infinite loop. Use \makeinline|file_SPLITAT| instead.
\end{enumerate}

Changing the configuration will trigger a recompilation of the files that are affected. For instance, changing \makeinline|file_SPLITAT| will cause \texttt{file/index.html} to rebuild, but not \texttt{file.pdf}, nor \texttt{notes/index.html}.

For more information about the Makefile syntax and how variables are evaluated, consult the \href{https://www.gnu.org/software/make/manual/}{GNU Make manual}.
...
\item[\texttt{CURL}] Command to call curl. Default \texttt{\textit{curl}} (if present).
\item[\texttt{DVISVGM}] Command to call dvisvgm. Default \texttt{\textit{dvisvgm}} (if present).
\item[\texttt{DVISVGMFLAGS}] Options to pass to dvisvgm. Default \texttt{\textit{--no-fonts --optimize}}.
\item[\texttt{EPSTOSVG\_CONVERTER}] Select which converter to use for converting EPS images to SVG. Currently supported values are \texttt{auto}, \texttt{dvisvgm}, \texttt{inkscape}; \texttt{auto} will look for dvisvgm, inkscape in that order. If set to empty, LaTeXML will convert EPS to PNG using ImageMagick. Default \texttt{\textit{auto}}.
\item[\texttt{INKSCAPE}] Command to call inkscape. Default \texttt{\textit{inkscape}} (if present).
\item[\texttt{INKSCAPEFLAGS}] Options to pass to inkscape. Default \texttt{\textit{--without-gui}}.
...
\item[\texttt{LATEXMLPOSTFLAGS}] Options to pass to \texttt{latexmlpost}.
\item[\texttt{MUTOOL}] Command to call mutool. Default \texttt{\textit{mutool}} (if present).
\item[\texttt{MUTOOLFLAGS}] Options to pass to mutool draw.
\item[\texttt{PDFTOCAIRO}] Command to call pdftocairo. Default \texttt{\textit{pdftocairo}} (if present).
\item[\texttt{PDFTOCAIROFLAGS}] Options to pass to pdftocairo.
\item[\texttt{PDFTOSVG\_CONVERTER}] Select which converter to use for converting PDF images to SVG. Currently supported values are \texttt{auto}, \texttt{dvisvgm}, \texttt{inkscape}, \texttt{mutool}, \texttt{pdftocairo}; \texttt{auto} will look for mutool, pdftocairo, dvisvgm, inkscape in that order. If set to empty, LaTeXML will convert PDF to PNG using ImageMagick. Default \texttt{\textit{auto}}.
...
\item[\texttt{UNZIP}] Command to call unzip. Default \texttt{\textit{tar}} (if present) on Windows, \texttt{unzip} (if present) on other platforms.
...
\subsection{Custom \CSS, Javascript, \ldots}\label{custom-css}
The \CSS{} files in the folder \texttt{bmluser} are automatically included at the end of the \htmlinline|<head>| tag and will override the previous styles. You may also include Javascript files (file extension \texttt{js}) and fonts (file extensions \texttt{ttf}, \texttt{woff}, \texttt{woff2})
...
Custom \CSS{} can also be added using \ltxinline|\bmlRawHTML{<style> ... </style>}| to the preamble, and likewise for \ltxinline|\bmlRawHTML{<script> ... </script>}|.
