...
\lstdefinestyle{bookml}{style=latexml,
  moretexcs=[2]{bmlImageEnvironment,bmlHTMLEnvironment,bmlRawHTML,bmlDescription,bmlPlusClass,bmlHTMLInlineEnvironment,bmlAltFormat,<}
}
...
\date{1\textsuperscript{st} July 2026}
...
\begin{itemize}
  ...
  \item Overleaf and GitHub integration: BookML can also be used with Overleaf by adding a single file to your project and linking it to GitHub (which requires a paid Overleaf subscription), or by directly putting your files on GitHub;
...
\cellcolor{red}\color{white} Required\phantom{:} & \cellcolor{red}\bmlRawHTML{} \\
...
\multicolumn{2}{X}{\textcolor{magenta}{\tt -{}-{}- PDFTOSVG\_CONVERTER is set to `mutool' -{}-{}-}} \\
  Ghostscript:   & \textcolor{OliveGreen}{9.25 OK} (BookML images, EPS to SVG, PDF to SVG via dvisvgm) \\
