...
The first line shows which files BookML will try to compile: each \texttt{.tex} file in the current folder that contains the string \ltxinline|\documentclass| (even if commented out!). The rest informs you on which software you may need to install or update. \textbf{Not everything is necessary:} the output of \cmdinline{make detect} will tell you what functionality requires a particular program. The \href{https://vlmantova.github.io/bookmlleeds/}{Leeds BookML guide} has detailed installation instructions for various platforms, many specific to the University of Leeds.

Finally, run \cmdinline{make} (or \cmdinline{gmake} on Windows). BookML will compile each `main file' to a zip file and a SCORM package. You can inspect the result by directly opening the folder \texttt{auxdir/html}.
  ...
  build:
    ...
    steps:
      - name: Compile with BookML
        uses: vlmantova/bookml-action@v1
    \end{lstlisting}
    I also recommend that you watch the repository (this is typically already happening for your own repository; see the `Watch' icon on the repository page, next to `Edit Pins', `Fork', `Star').
...
While many \texttt{.tex} files will be fine with no changes, you may want to tweak the output further by invoking the \ltxinline|\usepackage{bookml/bookml}| or by adding settings to \texttt{GNUmakefile}. Below are some notable examples. For the full list of options and commands, see the \hyperref[reference-manual]{Reference manual}.
...
\end{bmlimage} \]
  ...
  To deal with \textbf{every} \tikzname{} picture automatically, use \ltxinline|\bmlImageEnvironment{}|. The following code shows how to even \textbf{not load \tikzname}, so that \LaTeXML{} can run through the preamble faster, saving potentially up to 1 minute.
  \begin{lstlisting}[style=bookml]
\bmlImageEnvironment{tikzpicture}
\iflatexml
\else
...
\end{lxFooter}
    \end{lstlisting}
  The header is omitted in the bookdown style.
\end{foldedframe}
...
\begin{foldedframe}{Change MathJax version}\label{mathjax-versions}
  The current default for BookML is MathJax 3 due to remaining minor incompatibilities, but MathJax 4 already works well for most documents. If you want to jump ahead, use
  \begin{lstlisting}[style=bookml]
...
\bmlAltFormat{docs.large.pdf}{PDF (large print)}
  \end{lstlisting}
  Then BookML will compile \texttt{docs.large.tex} to PDF and include it in the \HTML{} output. The file will appear in the `Downloads' menu of the GitBook style.
...
\begin{foldedframe}{Some PDF and EPS figures look odd}\label{odd-pdf-eps-figures}
  BookML uses \texttt{dvisvgm} to convert PDF and EPS images to SVG. Unfortunately, that goes wrong under certain conditions. If that happens to you, please convert those figures yourself, and place the resulting images in the same place and with the same name as the original PDF or EPS files, but extensions \texttt{.svg}, \texttt{.png}, or \texttt{.jpg}.
\end{foldedframe}
...
\begin{foldedframe}{Some images look weird in sepia and dark mode}\label{images-in-sepia-dark-mode}
  ...
  To prevent a specific picture from changing, call \ltxinline|\bmlPlusClass{bml_no_invert}| immediately after the picture. You can see the difference on the two pictures below.
...
\begin{foldedframe}{Unsupported packages or classes}\label{unsupported-package-or-classes}
  ...
  \begin{itemize}
    ...
    \item If the package is producing images: use the same strategy as for \tikzname{}.
...
BookML can be configured by loading the \texttt{bookml/bookml} package and passing options to it like with any other LaTeX packages, for instance \ltxinline|\usepackage[imagescale=1.3333]{bookml/bookml}|. The following options are available:

\begin{description}
\item[\texttt{style=<\textit{name}>}] Switch style. <\textit{name}> can be \textit{gitbook} (default), which is almost identical to the output of bookdown, \textit{plain}, which is a tweaked version of the normal LaTeXML style, and \textit{none} for the default LaTeXML style with minimal compatibility and bug fixes only.
\item[\texttt{mathjax=<\textit{number}>}] Select which MathJax version to use from 2, 3 (default), 4 (\textbf{incomplete}).
\item[\texttt{nomathjax}] Disable MathJax.
\item[\texttt{imagescale=X.XX}] (Deprecated) Rescale all images generated via LaTeX (using \ltxinline|bmlimage|, see below) by the desired factor. Images are normally sized so that fonts inside the image match the font size of the browser, but there have been cases where BookML is wrong, and images turn out too small or too large. When that happens, tweak \textit{imagescale}, and please report the issue.
